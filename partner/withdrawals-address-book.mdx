...
<Steps>
  ...
  <Step title="Travel Rule only if the status asks">
    ...
    Required **only** when the accept **201** `status` is `pending-user-information`. Full field notes: [Travel Rule](/partner/travel-rule). Do not use Travel Rule v1. If requested and not sent, the operation stays in that status and is cancelled within a few hours. EUR threshold is not published here.
  </Step>
</Steps>
